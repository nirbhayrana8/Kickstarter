...
export default function Vote({ requests, data }) {
	...
	const executeTransaction = async (selectedRequestKey) => {
		...
		try {
			...
			const tx = await contract.approveRequest(requestIndex, options);
			data.setLoadingScreenText("Voting on request. Please wait.");
